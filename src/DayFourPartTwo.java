...
public class DayFourPartTwo {
    ...
    public static int findAnswer(List<String> input) {
        ...
        while (boards.size() > 1) {
            ...
            Board w = checkWinBoards(boards);
            while (w != null) {
                boards.remove(w);
                w = checkWinBoards(boards);
            }
        }
        while (!boards.get(0).checkWin()) {
            index++;
            boards.get(0).markField(Integer.parseInt(drawn[index]));
        }
        return boards.get(0).getSumUnmarked() * Integer.parseInt(drawn[index]);
    }
