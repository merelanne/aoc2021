...
//            System.out.println(from[0] + "," + from[1] + " -> " + to[0] + "," + to[1]);
            ...
            else if (Math.abs(Integer.parseInt(from[1])-Integer.parseInt(to[1]))
                    == Math.abs(Integer.parseInt(from[0]) - Integer.parseInt(to[0]))) {
//                System.out.println(from[0] + "," + from[1] + " -> " + to[0] + "," + to[1]);
                int fromX = Integer.parseInt(from[0]);
                int toX = Integer.parseInt(to[0]);
                int fromY = Integer.parseInt(from[1]);
                ...
                if (fromX < toX) {
                    if (fromY < toY) {
                        for (int i = fromY; i <= toY; i++) {
                            points[fromX + (i - fromY)][i]++;
                        }
                    } else {
                        for (int i = fromY; i >= toY; i--) {
                            points[fromX + (fromY - i)][i]++;
                        }
                    }
                } else {
                    if (fromY < toY) {
